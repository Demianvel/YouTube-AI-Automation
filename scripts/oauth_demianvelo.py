...
SECRET_NAME = "YOUTUBE_TOKEN_DEMIANVELO"
EXPECTED_HANDLE = "@demianvelo"
SCOPES = [
...
def _find_client_secret() -> Path:
    ...
    raise FileNotFoundError(
        "No encontre client_secret.json. Descarga una credencial OAuth tipo Desktop app de Google Cloud "
...
def _verify_channel(credentials: Credentials) -> tuple[str, str]:
    ...
    if handle.lower().lstrip("/") != EXPECTED_HANDLE:
        raise RuntimeError(
            f"Canal incorrecto. Se esperaba @DemianVelo y Google devolvio {handle or title}. "
            "No se guardo ningun token. Volve a ejecutar y elegi la cuenta/canal DemianVelo."
        )
    return handle, title
...
def main() -> None:
    ...
    _save_github_secret(credentials.to_json())

    with tempfile.NamedTemporaryFile("w", delete=False, prefix="demianvelo-token-check-", suffix=".json") as fh:
