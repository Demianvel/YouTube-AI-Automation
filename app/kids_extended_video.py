...
def _download_image(prompt: str, out: Path, seed: int) -> None:
    ...
    full_prompt = (
        ...
        + ", original premium 3D family animation, fictional characters only, rounded appealing shapes, "
          ...
          "vertical 9:16 composition, no text, no logo, no watermark, no real people, no copyrighted characters"
    )
...
def _image_clip(image: Path, out: Path, duration: int, index: int) -> None:
    ...
    vf = (
        ...
        f"zoompan=z='min(zoom+0.00045,1.075)':x='{x}':y='{y}':d={frames}:s={W}x{H}:fps={FPS},"
...
def _voice_segments(meta: dict, workdir: Path) -> list[Path]:
    ...
    for index, scene in enumerate(meta["scenes"], start=1):
        ...
        if not chunks:
            raise RuntimeError(f"Kokoro no genero voz para escena {index}.")
...
def render_extended_short(meta: dict, workdir: Path) -> Path:
    scene_seconds = int(meta["scene_seconds"])
    clips: list[Path] = []

    for index, scene in enumerate(meta["scenes"]):
        image = workdir / f"extended_image_{index + 1}.jpg"
        clip = workdir / f"extended_scene_{index + 1}.mp4"
        _download_image(scene["visual_prompt"], image, _seed(meta, index))
        _image_clip(image, clip, scene_seconds, index)
        image.unlink(missing_ok=True)
        clips.append(clip)

    manifest = workdir / "extended_visuals.txt"
    manifest.write_text("\n".join(f"file '{p.resolve()}'" for p in clips), encoding="utf-8")
    visual = workdir / "extended_visual.mp4"
    subprocess.run([
    ...
    voices = _voice_segments(meta, workdir)
    voice_manifest = workdir / "extended_voices.txt"
    voice_manifest.write_text("\n".join(f"file '{p.resolve()}'" for p in voices), encoding="utf-8")
    voice_full = workdir / "extended_voice.wav"
    subprocess.run([
